    ...
    public class RejBlockVwapMNQ : Strategy
    {
        ...
        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
                ...
                Description                  = "Rejection Block + VWAP + EMA + Partial TP on MNQ";
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (hour == 9 && minute == 30)
            {
                ...
                // reset consecutive losses every morning
                consecLosses   = 0;
                consecHalted   = false;
            }
                ...
                && c > emaVal)
            {
                ...
                Print(Time[0] + " LONG entry=" + c.ToString("F2")
                    + " sl=" + sl.ToString("F2")
                    + " partialTP=" + (c + initialRisk).ToString("F2")
        ...
        private void ManagePosition()
        {
            ...
            if (tradeSide == "long")
            {
                ...
                // ── Partial TP at 1:1 — exit 1 contract ───
                    ...
                    && Position.Quantity > 1)
                {
                    ...
                    Print(Time[0] + " PARTIAL TP long @ " + c.ToString("F2")
                        + " stop moved to breakeven " + stopPrice.ToString("F2"));
                }

                // ── Trail on remaining contract ────────────
                double profitPts = highWaterMark - entryPrice;

                if (profitPts >= trailActivation && !trailActive)
                {
                    trailActive = true;
                    Print(Time[0] + " TRAIL ACTIVATED long hwm=" + highWaterMark.ToString("F2"));
                }
                ...
                if (c <= stopPrice)
                {
                    ...
                    ExitLong("SL", "RB Long");
                    Print(Time[0] + (trailActive ? " TRAIL EXIT" : " SL")
                        + " long @ " + c.ToString("F2"));
                    tradeSide    = "";
                    partialTaken = false;
                }
            ...
            else if (tradeSide == "short")
            {
                ...
                {
                    trailActive = true;
                    Print(Time[0] + " TRAIL ACTIVATED short lwm=" + lowWaterMark.ToString("F2"));
                }
        ...
        private void UpdateConsecLosses(double pnl)
        {
            ...
            else
            {
                ...
                {
                    consecHalted = true;
                    Print(Time[0] + " CONSEC LOSS HALT — " + consecLosses + " in a row");
                }
            }
        }

        private void CheckMFFURules()
        {
            ...
            if (totalProfit > 0 && !dailyHalted)
            {
                ...
                if (pct >= consistencyLimit)
                {
                    ...
                    Print(Time[0] + " CONSISTENCY HALT " + (pct * 100).ToString("F1") + "%");
                }
            }
        }
    }
